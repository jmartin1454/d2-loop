...
# inconsistent with equations below which also rely on beta_t and
# rho_0.  If you want to use any other version, you must update beta_t
# and rho_0 rather than hard-coding your own Boussinesq formula.
...
# riser
##############################################
L_rise=L+L_down #top_z_hex-bottom_z-L_mod # m, height of rise
...
Tot_vol = (A*L) + (pi*D_down**2*L_down/4) + (pi*D_right**2*L_right/4) + (pi*D_down2**2*L_down2/4) + (pi*D_mod**2*L_mod/4) + (pi*D_left**2*L_left/4) + (pi*D_rise**2*L_rise/4)
...
for tstep in range(0,n_tsteps):
    ...
    for i in range(0,n_array):
        ...
        # new version by Jeff June 2, 2021
        if(Re<3000):
            Nu=3.657 # use circular tube correlation outside HEX
        else:
            fgniel=(0.79*log(Re)-1.64)**-2
            Pr=mu*cp/kt
            Nu=(fgniel/8)*(Re-1000)*Pr/(1+12.7*(fgniel/8)**.5*(Pr**(2/3)-1)) # https://en.wikipedia.org/wiki/Nusselt_number
        hcw=Nu*kt/D_h # Barron eq'n 6.15

        hcw=0 # remove heat transfer to/from walls
        
        # Do the heat xfer in this section
        if(i<n_hex):
            source_wall_to_fluid[i]=-4*hcw*(T_array[i]-T_wall[i])/(D_h*rho_0*cp)*(1-pcold/phelix)
        else:
            source_wall_to_fluid[i]=-4*hcw*(T_array[i]-T_wall[i])/(D_h*rho_0*cp)

        if(i<n_hex):
            source_fluid_to_wall[i]=4*hcw*(T_array[i]-T_wall[i])/(D_h*rhocp_wall[i])*(1-pcold/phelix)
        else:
    ...
    for i in range(0,n_hex):
        ...
        if(Re<3000):
            ...
            Pr=mu*cp/kt
            Nu=(fgniel/8)*(Re-1000)*Pr/(1+12.7*(fgniel/8)**.5*(Pr**(2/3)-1)) # https://en.wikipedia.org/wiki/Nusselt_number
        hc=Nu*kt/D_h # Barron eq'n 6.15

        source_array[i]=-4*hc*(T_array[i]-T_cold)/(D_hex*rho_0*cp)*pcold/phelix
